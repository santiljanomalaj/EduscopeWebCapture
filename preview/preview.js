...
function onGettingFile(f, item) {
    ...
    (function() {
        ...
        hidden.onloadedmetadata = function() {
            ...
            console.log('hidden',hidden.clientHeight,hidden.clientWidth,);
            fresolutions.innerHTML = 'Resolution: ' +  hidden.clientWidth + 'x' + hidden.clientHeight;

            if(hidden.duration === Infinity) {
                setTimeout(hidden.onloadedmetadata, 1000);
                return;
            }
            fduration.innerHTML ='Duration: ' +  formatSecondsAsTime(hidden.duration);
    ...
    fsize.innerHTML = 'File size: ' + bytesToSize(file.size);
    ...
    var html = 'Your recorded video file is currently stored in the browser memory. Click <a href="' + file.url + '" download="' + file.name + '">here</a> to download and save.';
