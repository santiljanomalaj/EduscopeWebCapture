...
function captureCamera(callback, defaultDevices) { 
    ...
    // }
    console.log('camera config',constraints)
    navigator.mediaDevices.getUserMedia(constraints).then(function(stream) {
        initVideoPlayer(stream);
        callback(stream);
        console.log('media',openCameraPreviewDuringRecording, enableScreen, enableCamera);
        ...
        if (enableCamera && enableScreen && openCameraPreviewDuringRecording) {
            chrome.windows.create(
                {url:'camera.html', type:'popup', focused:true, width:380, height:290,left:0,top:0}, function(Window) {
                    console.log('camerawindowsid',Window.id);
